    ...
    if st.session_state['view'] == 'Overtime':
        ...
        with col1:
            ...
            if not vectors_count_today.empty:
                st.dataframe(vectors_count_today, use_container_width=True)
            else:
        ...
        avg_entailment_score = qa_df['Entailment'].mean()
        number_of_flags = qa_df['flag'].sum()

        col1, col2, col3, col4 = st.columns(4)
        st.title('Base model Q&A Tracking')
        with col1:
            st.subheader('Q&A Accuracy Score')
            if Accuracy < 50:
        ...
        with col3:
            ...
            st.write(f"Total Questions: {qa_df.shape[0]}")
        with col4:
